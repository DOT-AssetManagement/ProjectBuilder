...
namespace PBLogicTest
{
    ...
    public class MixedLogicText
    {
        ...
        [TestMethod]
        public void TestExtendedOptimization()
        {
           
             bool  ok = SymphonyConductor.RunExtendedScenario(_ConnectionString, _homeDirectory, 9,
                    ...
                    , 2, true, out string errorMessage);
        ...
        {
            bool ok = DataManager.PopulateScenarioTreatmentsTable(_ConnectionString, 1, out string errorMessage);

            if (ok)
            {
                 ok = DataManager.CreateScenarioProjects(_ConnectionString, 1, true,
                       true, true, true, true, true, true, out errorMessage);
            }

            if (ok)
            {
                ok = SymphonyConductor.RunMixedScenarioFull(_ConnectionString, _homeDirectory, 1,
                    new ScenarioCommunique
                    {
                        Commitment = false,
                        District = null,
            ...
            if (ok)
            {
                ok = DataManager.CreateScenarioProjects(_ConnectionString, 1, false,
                  true, true, true, true, true, true, out  errorMessage);
            }
        ...
        [TestMethod]
        public void TestCreateExtendedScenarioProjects()
        {
            bool ok = DataManager.CreateExtendedScenarioProjects(_ConnectionString, 9, out string errorMessage);
            Assert.IsTrue(ok, errorMessage);
        }
