    ...
    class Program
    {
        ...
        private static log4net.ILog ConfigureLogger()
        {
            ...
            log4net.Appender.RollingFileAppender appender = new RollingFileAppender
            {
                ...
                File = @".\Logs\PAMSImport.log" // all logs will be created in the subdirectory logs 
            };
            ...
            log4net.ILog log = log4net.LogManager.GetLogger("PAMSImport");

            log.Debug("PAMSImport logger created.");
...
>PAMSImport[.exe] [/T] [/I] [/?|/H]
- /T - to truncate tables in the target JPAMS database;
- /I - to import data from PAMS database into JPAMS;
- /? or /H- to display this message.

When both  /T and /I arguments are used then target tables are truncated and then populated importing data from PAMS.
...
";
        ...
        {
            Log.Info("PAMSImport started...");
            string configFileName = "config.json";

            PAMSDataImporter.ImportManager.Log = Log;
            ...
            if (ok && _truncate)
            {
                ok = PAMSDataImporter.ImportManager.TruncateJpAndPamsTables(configFileName, out errorMessage);
            }

            if (ok && _import)
            {
                ok = PAMSDataImporter.ImportManager.PAMS2JPAMS(configFileName, out errorMessage);
            }
